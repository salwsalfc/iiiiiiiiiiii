...
ask_ques = "𓅄𓅄• ذا كنت تريد تنصب ميوزك اختار بايروجرام\n\n• واذا تريد تنصب تليثون فأختار تيرمڪس\n\n• يوجد استخرجات جلسات لي البوتات𓅄𓅄"
...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if is_bot:
        ty += " ʙᴏᴛ"
    await msg.reply(f"» ⚡ ¦ بـدء إنـشـاء جـلسـة 𓅄𓅄{ty}𓅄𓅄 ...")
    user_id = msg.chat.id
    ...
    if api_id_msg.text == "/skip":
        ...
        try:
            ...
            await api_id_msg.reply("𓅄𓅄ᴀᴩɪ_ɪᴅ𓅄𓅄 ᴍᴜsᴛ ʙᴇ ᴀɴ ɪɴᴛᴇɢᴇʀ, sᴛᴀʀᴛ ɢᴇɴᴇʀᴀᴛɪɴɢ ʏᴏᴜʀ sᴇssɪᴏɴ ᴀɢᴀɪɴ.", quote=True, reply_markup=InlineKeyboardMarkup(gen_button))
            return
    ...
    if not is_bot:
        ...
        t = "ᴩʟᴇᴀsᴇ sᴇɴᴅ ʏᴏᴜʀ 𓅄𓅄ʙᴏᴛ_ᴛᴏᴋᴇɴ𓅄𓅄 ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ.\nᴇxᴀᴍᴩʟᴇ : `5432198765:abcdanonymousterabaaplol`'"
    ...
    if not is_bot:
        ...
        try:
            ...
            await msg.reply("» ᴛʜᴇ ᴏᴛᴩ ʏᴏᴜ'ᴠᴇ sᴇɴᴛ ɪs 𓅄𓅄ᴡʀᴏɴɢ.𓅄𓅄\n\nᴩʟᴇᴀsᴇ sᴛᴀʀᴛ ɢᴇɴᴇʀᴀᴛɪɴɢ ʏᴏᴜʀ sᴇssɪᴏɴ ᴀɢᴀɪɴ.", reply_markup=InlineKeyboardMarkup(gen_button))
            return
        except (PhoneCodeExpired, PhoneCodeExpiredError):
            await msg.reply("» ᴛʜᴇ ᴏᴛᴩ ʏᴏᴜ'ᴠᴇ sᴇɴᴛ ɪs 𓅄𓅄ᴇxᴩɪʀᴇᴅ.𓅄𓅄\n\nᴩʟᴇᴀsᴇ sᴛᴀʀᴛ ɢᴇɴᴇʀᴀᴛɪɴɢ ʏᴏᴜʀ sᴇssɪᴏɴ ᴀɢᴀɪɴ.", reply_markup=InlineKeyboardMarkup(gen_button))
            return
    ...
    text = f"𓅄𓅄جلستك يبروو {ty} sᴛʀɪɴɢ sᴇssɪᴏɴ𓅄𓅄 \n\n`{string_session}` \n\n𓅄𓅄ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ :𓅄𓅄 @wasit_go \n🍒 𓅄𓅄ɴᴏᴛᴇ :𓅄𓅄 حافظ عليها ممكن حد يخترقكك بيها\n انضم هناا فضلا @wasit_go 🥺"
    try:
        if not is_bot:
    ...
    await bot.send_message(msg.chat.id, "» ✅تم استخراج الجلسه بنجاح ️ {} .\n\n🔍من فضلك اذهب الي الرسايل المحفوظه بحسابك!  ! \n\n𓅄𓅄ᴀ sᴛʀɪɴɢ ɢᴇɴᴇʀᴀᴛᴏʀ ʙᴏᴛ ʙʏ𓅄𓅄 @wasit_go 🥺".format("ᴛᴇʟᴇᴛʜᴏɴ" if telethon else "ᴩʏʀᴏɢʀᴀᴍ"))


async def cancelled(msg):
    if "/cancel" in msg.text:
        await msg.reply("𓅄𓅄 تم تنفيـذ طـلبگ آنتهہ‏‏ت آلعمـليـهہ‏‏ لبدآيـ‏‏هہ آگتب /start !𓅄𓅄", quote=True, reply_markup=InlineKeyboardMarkup(gen_button))
        return True
    elif "/restart" in msg.text:
        await msg.reply("𓅄𓅄»تم اعادة التشغيل من لاول !𓅄𓅄", quote=True, reply_markup=InlineKeyboardMarkup(gen_button))
        return True
    elif "/skip" in msg.text:
        return False
    elif msg.text.startswith("/"):  # Bot Commands
        await msg.reply("𓅄𓅄» ᴄᴀɴᴄᴇʟʟᴇᴅ ᴛʜᴇ ᴏɴɢᴏɪɴɢ sᴛʀɪɴɢ ɢᴇɴᴇʀᴀᴛɪᴏɴ ᴩʀᴏᴄᴇss !𓅄𓅄", quote=True)
        return True
    else:
        return False
